 ...
 <script>
     ...
     function accPurchasingStatusChange(url) {
         Swal.fire({
             title: 'Confirmation Request',
             text: 'Anda yakin ingin melanjutkan ke bagian finance untuk di lakukan pembayaran?',
             icon: 'success',
             showCancelButton: true,
             confirmButtonColor: '#800080',
             cancelButtonColor: '#FFA500',
     ...
     function accFinanceStatusChange(url, invoice_number, nama_karyawan, nama_brg, qty, harga, deadline_at) {
         // Mendapatkan tanggal saat ini
         var currentDate = new Date();
         // Parsing tanggal deadline_at dari string ke objek Date
         var deadlineDate = new Date(deadline_at);

         // Menghitung selisih waktu dalam milidetik
         var timeDifference = currentDate - deadlineDate;

         // Menghitung selisih hari (1000 ms * 60 detik * 60 menit * 24 jam)
         var daysDifference = Math.floor(timeDifference / (1000 * 60 * 60 * 24));

         // Cek jika selisih waktu lebih dari satu hari
         if (daysDifference > 1) {
             Swal.fire({
                 title: 'Gagal',
                 text: 'Pembayaran sudah melebihi satu hari sejak tanggal deadline.',
                 icon: 'error',
                 confirmButtonColor: '#800080'
             });
         } else {
             Swal.fire({
                 title: 'Confirmation Payment<hr class="dashed">',
                 html: '<div class="form-group">' +
                     '<label for="invoice_number">Invoice ID:</label>' +
                     '<input type="text" class="form-control" id="invoice_number" value="' + invoice_number + '" readonly>' +
                     '</div>' +
                     '<div class="form-group">' +
                     '<label for="nama_karyawan">Nama Pengaju:</label>' +
                     '<input type="text" class="form-control" id="nama_karyawan" value="' + nama_karyawan + '" readonly>' +
                     '</div>' +
                     '<div class="form-group">' +
                     '<label for="nama_brg">Nama Barang:</label>' +
                     '<input type="text" class="form-control" id="nama_brg" value="' + nama_brg + '" readonly>' +
                     '</div>' +
                     '<div class="form-group">' +
                     ...
                     '<input type="text" class="form-control" id="harga" value="' + harga + '" readonly>' +
                     '</div>' +
                     '<div class="form-group">' +
                     '<label for="deadline_at">Deadline Pembayaran:</label>' +
                     '<input type="text" class="form-control" id="deadline_at" value="' + deadline_at + '" readonly>' +
                     '</div>',
                 icon: 'success',
                 showCancelButton: true,
                 confirmButtonColor: '#800080',
 ...
 <script>
     ...
     selectKaryawan.on('change', function() {
         ...
         // Menggunakan jQuery AJAX untuk mengambil data terkait
         $.ajax({
             url: "<?php echo site_url('others/payslip/getKaryawanDetail/') ?>" + selectedValue,
             method: 'GET',
             dataType: 'json',
             success: function(data) {
